...
function start(){startBattle(state);paused=false;lastPhase='';canvas.focus();}
...
function sync(){
  ...
  el('command').textContent=paused?'Paused. Pip can wait.':state.lastCommand;
  ...
  if(phase==='paused'){h.textContent='Taking a breather.';p.textContent='Your battle is right where you left it.';actions.append(button('Resume battle',()=>{paused=false;canvas.focus();},true),button('New run',newRun));}
  if(phase==='reward'){
    ...
    for(const id of state.choiceIds){const u=UPGRADES.find(x=>x.id===id),b=button(u.name,()=>{if(chooseUpgrade(state,id))start();});const small=document.createElement('small');small.textContent=u.detail;b.append(small);actions.append(b);}
  }
